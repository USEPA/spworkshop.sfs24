...
Prior to the start of the workshop everyone will need to have the software installed and tested. You will need to have both R (at least version 4). Get the latest versions of each and install using the defaults: [R CRAN](https://cran.r-project.org/).

In addition to R, it will be critical to install the compatible version of RTools (also available at [R CRAN](https://cran.r-project.org/). This will allow you to install the GitHub version of pakcages that are not yet on the CRAN (see below).
