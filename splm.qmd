...
* Construct and describe the spatial linear model
    * Review the linear model with independent random errors
...
The `glances()` function allows us to compare the model fit statistics for a few different models simultaneously. Two of these fit statistics are AIC and AICc, which are commonly used to select a "best" model -- the lower the AIC/AICc, the better the model. `glances()` automatically orders the models by AICc:
```{r}
glances(spmod, none)
```
